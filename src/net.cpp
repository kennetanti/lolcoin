...
void ThreadMapPort()
{
    ...
    int error = 0;
#ifdef WIN32
    devlist = upnpDiscover(2000, multicastif, minissdpdpath, 0, 0, &error, NULL);
#else
    ...
    if (r == 1)
    {
        ...
        string strDesc = "lolcoin " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. lolcoin is probably already running."), addrBind.ToString().c_str());
        else
